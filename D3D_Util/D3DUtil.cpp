...
void SetDebugLayerInfo(ID3D12Device* pD3DDevice)
{
	...
	pD3DDevice->QueryInterface(IID_PPV_ARGS(&InfoQueue));
	if (InfoQueue)
	{
...
void D3DUtil::UpdateTexture(ID3D12Device* pD3DDevice, ID3D12GraphicsCommandList* pCommandList, ID3D12Resource* pDestTexResource, ID3D12Resource* pSrcTexResource)
{
...
char* ConvertWCtoC(const wchar_t* wcharStr) {
	// 필요한 버퍼 크기 계산
	int bufferSize = WideCharToMultiByte(CP_UTF8, 0, wcharStr, -1, nullptr, 0, nullptr, nullptr);
	if (bufferSize == 0) {
		return nullptr; // 변환 실패 시 null 반환
	}

	// 멀티바이트 문자열을 저장할 버퍼 할당
