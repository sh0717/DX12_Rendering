...
void CBasicMeshObject::Draw(ID3D12GraphicsCommandList* pCommandList,  XMMATRIX* pWorldMatrix )
{
	...
	if (pCommandList && IsInitialized())
	{
		...
		for(UINT i = 0 ; i < m_TriGroupCount ; ++i)
		{
			...
			{
				/*#ifdef _DEBUG
							std::cerr << "CBasicMeshObject::Draw(ID3D12GraphicsCommandList* pCommandList,  XMMATRIX* pWorldMatrix )		Trigroup Data 에 TextureHandle* 이 null 이라서 Default Texture 를 사용합니다";
				#endif*/
